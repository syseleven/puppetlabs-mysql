...
describe 'mysql_grant' do
  ...
  describe 'proxy privilieges' do
    ...
    describe 'adding proxy privileges with mysql version less than 5.5.0', unless: version_is_greater_than('5.5.0') do
      ...
      it 'does not find the user' do
        expect(run_shell('mysql -NBe "SHOW GRANTS FOR proxy2@tester"', expect_failures: true).stderr).to match(%r{There is no such grant defined for user 'proxy2' on host 'tester'})
      end
    end
